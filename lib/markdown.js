...
const MarkdownConverter = {
  ...
  renderContentBlock(block, msg, options) {
    ...
    switch (block.type) {
      ...
      case 'tool_use': {
        const isCodeExecution = this.isCodeExecutionTool(block);
        if (isCodeExecution) {
          // Code execution (bash, python, etc.) → controlled by "include thinking"
          if (!options.includeThinking) return null;
        } else {
          // Document / research / artifact → controlled by "include artifacts"
          if (!options.includeArtifacts) return null;
        }
  ...
  renderToolUse(block, options) {
    ...
    // Handle "artifacts" tool directly — content is always in input
    if (block.name === 'artifacts' && block.input?.content) {
      ...
      // Markdown/text artifacts → plain text; code artifacts → code block
    ...
    if (dc) {
      // Code blocks (artifacts with language)
      ...
      // Text / Markdown / Document (research reports, etc.)
      ...
      // Any other display_content with text/content
      const fallbackContent = dc.content || dc.text || '';
      if (fallbackContent) {
        const title = block.input?.title || '';
        const header = title ? `### ${title}\n\n` : '';
        return `${header}${fallbackContent}`;
      }
    }
    ...
    if (block.input) {
      ...
      if (content) {
        ...
        const inputType = block.input.type || '';
        const header = title ? `### ${title}\n\n` : '';
        // Markdown/text content → render as plain text
        if (!lang || inputType.includes('markdown') || inputType.includes('text') || lang === 'markdown') {
          return `${header}${content}`;
        }
        // Code content → render as code block
        return `${header}\`\`\`${lang}\n${content}\n\`\`\``;
      }
    }
  ...
  // Check if a tool_use block is code execution (vs document/artifact)
  // Conservative: only return true when we're confident it's code execution
  isCodeExecutionTool(block) {
    const name = (block.name || '').toLowerCase();

    // Known code execution tools
    if (/^(bash|python|computer|terminal|shell|execute|run_code|code_execution|ipython|bash_tool)/.test(name)) {
      return true;
    }

    // Known NON-code tools — always return false
    if (/^(artifacts|web_search|image_search|launch_extended_search|ask_user|memory_user|chart_display|view)/.test(name)) {
      return false;
    }

    // Code block without title/filename = execution output
    const dc = block.display_content;
    if (dc && (dc.type === 'code_block' || dc.type === 'code')) {
      if (!dc.filename && !block.input?.title) return true;
    }

    // Default: not code execution
    return false;
  },
