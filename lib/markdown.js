...
const MarkdownConverter = {
  ...
  isCodeExecutionTool(block) {
    ...
    if (/^(bash|python|computer|terminal|shell|execute|run_code|code_execution)/.test(name)) {
      return true;
    }

    // Check display_content type - code blocks are execution
    const dc = block.display_content;
    if (dc && (dc.type === 'code_block' || dc.type === 'code')) {
      // If there's no title/filename, it's likely execution output, not an artifact
      if (!dc.filename && !block.input?.title) return true;
    }

    // If no display_content but input has raw code/command, it's execution
    if (!dc && block.input) {
      const input = block.input;
      if (input.command || input.code) return true;
      // No title suggests it's not a user-facing artifact
      if (!input.title && input.content) return true;
    }
