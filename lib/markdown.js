...
const MarkdownConverter = {
  ...
  renderToolUse(block, options) {
    ...
    if (block.input) {
      ...
      if (content) {
        ...
        const header = title ? `### ${title}\n\n` : '';
        // If it has a language, it's code; otherwise render as plain text
        if (lang) {
          return `${header}\`\`\`${lang}\n${content}\n\`\`\``;
        }
        return `${header}${content}`;
  ...
  isCodeExecutionTool(block) {
    // Check tool name for known code execution tools
    const name = (block.name || '').toLowerCase();
    if (/^(bash|python|computer|terminal|shell|execute|run_code|code_execution|ipython)/.test(name)) {
      return true;
    }

    // Check if input has explicit command/code fields
    if (block.input?.command || block.input?.code) {
      return true;
    }

    // Code block without title/filename = execution output (not a named artifact)
    const dc = block.display_content;
    if (dc && (dc.type === 'code_block' || dc.type === 'code')) {
      if (!dc.filename && !block.input?.title) return true;
    }

    // Everything else (research, documents, artifacts, web_search) is NOT code execution
    return false;
  },
